...
class Hv:
    ...
    def init_the_visual_detection(self, camera_id=0):
        data = http_post("http://{}:{}/initCamera".format(self.ip, self.port), {"camera_id": camera_id})
        print(data)

    # init the ocr core
    # no param
    def init_the_ocr(self):
        data = http_post("http://{}:{}/initOCR".format(self.ip, self.port), {})
        print(data)
    ...
    # @param "icon_full_path" is the full path in master pc of the icon you prepare to upload,
    # etc "/home/xxx/xxx/icon.png"
    ...
    def upload_icon(self, icon_full_path, keyFn, scale, Volume):
        data = http_post("http://{}:{}/loadIcon".format(self.ip, self.port),
                         {"keyFn": keyFn, "scale": scale, "Volume": Volume},
                         post_files={"iconFile": open(icon_full_path, "rb")})
        print(data)

    # delete the icons that has been uploaded to the server in progress memory
    def del_all_target(self):
        data = http_post("http://{}:{}/delAllTarget".format(self.ip, self.port), {})
        print(data)
    ...
    def capture_one_current_frame(self, save_full_path):
        data = http_post("http://{}:{}/capture".format(self.ip, self.port), {})
        if data["image"]:
    ...
    # notice! only support jpg image file
    def find_icon_in_frame(self, icon_name, OV, if_save_show_image, save_full_path=""):
        data = http_post("http://{}:{}/findIcon".format(self.ip, self.port),
                         {"iconName": icon_name, "OV": OV, "saveShow": if_save_show_image})
    ...
    # no param
    def find_text(self):
        data = http_post("http://{}:{}/findText".format(self.ip, self.port), {})
        print(data)

        # find the text_box_position
        # text_box_position format : list : [["text1", cx, cy], ["text2", cx, cy], ["text3", cx, cy], ...],
        # cx, cy is pixel index
        ret = []
    ...
    def measure_the_pixel_point(self, pts, fx=641.908787, cx=688.383484, fy=639.740430, cy=374.862427, Zc=245.0):
        ret = []
        for p in pts:
            rdr = http_post("http://{}:{}/measureXY".format(self.ip, self.port),
                            {"x": p[0], "y": p[1], "fx": fx, "cx": cx, "fy": fy, "cy": cy, "Zc": Zc})
            ret.append(rdr["xyz"])
        print(ret)
    ...
    def get_device_size(self, icon_name, OV, shape):
        size_info_ret = []

        rect_data = self.find_icon_in_frame(icon_name, OV, 1, "capture_save.jpg")
        if not rect_data:
        ...
        for pd in rect_data:
            ...
            if shape == "square":
                ...
                m_ret = self.measure_the_pixel_point([[x1, y1], [x2, y2]])
                ...
                size_info['info'] = {"x": (xx1 + xx2) / 2, "y": (yy1 + yy2) / 2,
                                     "radius": (abs(xx2 - xx1) + abs(yy2 - yy1)) / 4}
    ...
    # @param "OV" is the confidence threshold of icon detection
    def find_icon_in_frame_and_measure_them(self, icon_name, OV):
        rect_data = self.find_icon_in_frame(icon_name, OV, 1, "capture_save.jpg")
        if not rect_data:
            return []

        pts_data = [[pp[0], pp[1]] for pp in rect_data]
        m_ret = self.measure_the_pixel_point(pts_data)
